...
bool angle_test(Mat image,QProgressDialog *Progress,string mode = "test", bool use_rot = true){
line2Dup::Detector detector(128, {4, 8});

    if(mode != "test"){
        //isOnce = true;
        //Mat img = imread(prefix+"case1/train.png");
        assert(!image.empty() && "check your img path");

        Rect roi(130, 110, 270, 270);
        image = image(roi).clone();
        Mat mask = Mat(image.size(), CV_8UC1, {255});

        // padding to avoid rotating out
        int padding = 100;
        cv::Mat padded_img = cv::Mat(image.rows + 2*padding, image.cols + 2*padding, image.type(), cv::Scalar::all(0));
        image.copyTo(padded_img(Rect(padding, padding, image.cols, image.rows)));

        cv::Mat padded_mask = cv::Mat(mask.rows + 2*padding, mask.cols + 2*padding, mask.type(), cv::Scalar::all(0));
        mask.copyTo(padded_mask(Rect(padding, padding, image.cols, image.rows)));
        ...
        //Mat img = imread(prefix+"test.png");
        assert(!image.empty() && "check your img path");
...
//        cvtColor(img, img, CV_BGR2GRAY);
        ...
        qDebug() << "test img size: " << img.rows * img.cols << endl;
        ...
        qDebug()<< "matches.size(): " << matches.size() << endl;
        size_t top5 = 1;
        if(top5>matches.size()) top5=matches.size();
        for(size_t i=0; i<top5; i++){
            auto match = matches[i];
            ...
            qDebug() << "\nmatch.template_id: " << match.template_id << endl;
            qDebug() << "match.similarity: " << match.similarity << endl;
        }

        //imshow("img", img);
        //waitKey(1);
        qDebug() << "test end" << endl;
    }
    return true;
}
